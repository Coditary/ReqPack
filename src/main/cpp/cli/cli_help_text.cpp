...
std::string general_help_text() {
    return PROGRAM_NAME + " - Unified Package Manager Interface\n"
           ...
           "\nSecurity:\n"
           "  --audit                 Enable vulnerability checks and security metadata scans\n"
           "  --no-security           Disable vulnerability checks (overrides config and --audit)\n"
    ...
    switch (action) {
        case ActionType::INSTALL:
            return
                ...
                "  --json                  Emit machine-readable JSON result on stdout\n"
                "  --audit                 Run vulnerability checks before installing\n"
                "  --no-security           Disable vulnerability checks (default)\n"
                "  --prompt-on-unsafe      Prompt before installing vulnerable packages\n"
                "  --abort-on-unsafe       Abort if vulnerable packages are found\n"
                "  --severity-threshold    Minimum severity to flag (low/medium/high/critical)\n"
                "  --score-threshold       Minimum CVSS score to flag (0.0-10.0)\n"
        ...
        case ActionType::UPDATE:
            return
                ...
                "Use 'rqp update --all' to refresh all installed plugin wrappers.\n"
                ...
                "  --dry-run               Show planned actions without executing them\n"
                "  --json                  Emit machine-readable JSON result on stdout\n"
                "  --all                   Update all packages for system, or all installed plugin wrappers without a system\n"
                "  --audit                 Run vulnerability checks before updating\n"
                "  --no-security           Disable vulnerability checks (default)\n"
