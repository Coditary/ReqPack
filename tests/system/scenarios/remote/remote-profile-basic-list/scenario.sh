...
scenario_client_command() {
    local body
    body="cd $(shell_quote "${WORKSPACE_DIR}") && rqp --config $(shell_quote "${CONFIG_PATH}") remote dev list quill && rqp --config $(shell_quote "${CONFIG_PATH}") remote admin shutdown >/dev/null"
    command_with_wait_prefix "${body}"
}

scenario_assert() {
    assert_equals "0" "$(read_status)" "remote basic list exits zero"
