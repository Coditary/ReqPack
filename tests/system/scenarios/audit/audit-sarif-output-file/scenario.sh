...
scenario_command() {
    printf 'cd %q && rqp --config %q audit quill:zeno --format sarif --output audit.sarif' "${WORKSPACE_DIR}" "${CONFIG_PATH}"
}

scenario_assert() {
    assert_equals "0" "$(read_status)" "audit sarif export exits zero"
