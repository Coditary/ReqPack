...
scenario_command() {
    printf 'cd %q && rqp --config %q update quill --severity-threshold high' "${WORKSPACE_DIR}" "${CONFIG_PATH}"
}

scenario_assert() {
    assert_equals "0" "$(read_status)" "severity threshold update exits zero"
