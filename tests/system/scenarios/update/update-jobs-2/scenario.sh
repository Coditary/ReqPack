...
case "${cmd}" in
  update)
    ...
    ;;
  update-all)
    printf "start quill all\n" >> "${CALLS_DIR}/parallel.log"
    sleep 1
    printf "end quill all\n" >> "${CALLS_DIR}/parallel.log"
    ;;
esac
'
}
...
scenario_command() {
    printf 'cd %q && rqp --config %q update quill:zeno moss:luma --jobs 2' "${WORKSPACE_DIR}" "${CONFIG_PATH}"
}

scenario_assert() {
    assert_equals "0" "$(read_status)" "update jobs 2 exits zero"
